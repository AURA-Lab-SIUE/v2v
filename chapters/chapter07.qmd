...
![](../images/ch07-crossroads_3.jpg){fig-alt="A stylized rendering of concentric sound-wave forms suggesting close attention."}
...
**Two-coder reliability planning begins here.** The undergrad edition describes structured immersion as the process of watching or listening to media with focused attention before building a codebook: you take field notes, identify candidate categories, and notice edge cases. The graduate edition adds a second lens — you are simultaneously identifying *where two coders will disagree*.

**Why disagreement identification matters before the codebook.** Most content-analytic codebook failures occur not from poorly defined categories but from underspecified decision rules for boundary cases. When you watch a Twitch stream and code a comment as "hostile," another coder watching the same stream might code it as "sarcastic" — because the category boundary was not drawn precisely enough. Structured immersion is when this ambiguity first becomes visible. The graduate-level field notes document not just *what* you observe, but *where* you are uncertain, because those locations are exactly where a second coder will need explicit guidance.

**Planning for the reliability pilot.** The formal intercoder reliability protocol (Chapter 8) requires a training phase and a reliability test on a separate data subset. The training phase consists of two coders applying the codebook independently to the same units, then reconciling disagreements. Effective training requires the genuinely ambiguous cases you identified during immersion. Your graduate-level field notes double as a training-set seed: the hardest-to-categorize observations become the calibration material.

**Required reading.** Krippendorff, K. (2018). *Content analysis: An introduction to its methodology* (4th ed.). SAGE Publications.

**Reflect.** (1) After completing your immersion log, identify three categories where you anticipate the most disagreement with a second coder. For each, write one candidate decision rule that would resolve the ambiguity. (2) Krippendorff argues that unitizing — deciding what counts as a single unit of analysis — is a prior reliability problem most researchers underestimate. What is your unit of analysis, and what edge cases make consistent unitizing difficult?
:::
:::
## References
