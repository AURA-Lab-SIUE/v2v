...
```{=html}
<audio class="v2v-audio v2v-audio-ug" controls preload="none"><source src="../audio/ch09-ug.mp3" type="audio/mpeg">Your browser does not support audio.</audio>
<audio class="v2v-audio v2v-audio-grad" controls preload="none"><source src="../audio/ch09-grad.mp3" type="audio/mpeg">Your browser does not support audio.</audio>
```
...
$ channel <chr> "domingo", "zanoxvii", "kendinemuzisyen", "xqcow", "kendinemuzisyen", …
...
Five columns, 157,080 rows, and each row is one chat message. `id` is a number that uniquely labels the message. `channel` is the channel the message was sent in. `sender` is the account that sent it. `message` is the text itself. `date` is when it was sent.
...
$ channel <chr> "btssmash", "sodapoppin", "xqcow", "domingo", "forsen", "zanoxvii…
...
Six columns, 90,245 rows, and each row is one snapshot of one stream at one moment: its `title`, the `game` it was filed under, its `viewers` count, and the `date` of the snapshot. The second row should look familiar. It is Sodapoppin's channel, playing Marble It Up!, with 27,934 viewers, the exact row Chapter 2 used to open its discussion of reproducible data. You are now looking at the source of that example directly, sitting among the other 228 channels the sample covers.
...
![](../images/fig09-1-table-shape.png){fig-alt="Two horizontal bar panels profile the chat table of 157,080 rows and the streams table of 90,245 rows, one bar per column showing the percentage of values present, with each column's type in parentheses. Every column of the chat table is complete. In the streams table 96 snapshots are missing a title and 450 are missing a game category. Computed from the shipped sample data."}
