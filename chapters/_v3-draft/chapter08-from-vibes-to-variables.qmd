...
```{=html}
<audio class="v2v-audio v2v-audio-ug" controls preload="none"><source src="../../audio/ch08-ug.mp3" type="audio/mpeg">Your browser does not support audio.</audio>
<audio class="v2v-audio v2v-audio-grad" controls preload="none"><source src="../../audio/ch08-grad.mp3" type="audio/mpeg">Your browser does not support audio.</audio>
```

:::


Somewhere in the field-notes document you built working through the *Structured Listening* supplement is an entry like this one: "Chat in the art stream felt calmer and more conversational than chat in the gaming stream. People talked to each other, not just to the streamer." That sentence is a real observation. You earned it by watching. It is also, as written, useless to a coder.
...
Take the variable the running example has been circling since Chapter 6: the target of a chat message, who it is aimed at. That observation complicated the simple two-way split, so call the categories directed at the streamer, broadcast to the room, directed at another viewer, and unclassifiable.
...
The `stream_log` table is a convenient place to see all four, because its columns happen to span the range. Recall the preview in *The Open Workspace*: channel, title, game, viewers, date.
...
![](../../images/fig08-1-levels-of-measurement.png){fig-alt="Four panels measure stream titles at different levels. Nominal: five example titles, drawn from the 613 distinct titles the fixture holds across 90,245 snapshots, so counting categories reveals little. Ordinal: titles binned by length into short, medium and long, with the medium bin holding the median. Ratio: a histogram of title length in characters with a mean of 48.5, where a true zero makes every statistic available. Interval: daily chat-message counts across the week, standing in for the timestamp, which has equal spacing but an arbitrary zero; a raw string can never be measured at the interval level."}
...
![](../../images/fig08-2-reliability-validity.png){fig-alt="Four target diagrams, each with a cross at the bullseye standing for the concept being measured and dots standing for repeated measurements. Reliable and valid: dots cluster tightly on the bullseye. Reliable but not valid: dots cluster just as tightly but well off-centre, consistent and consistently wrong. Valid on average but not reliable: dots scatter widely around the centre, right on average and unusable one at a time. Neither: dots scatter widely and off-centre. A schematic illustration, not data."}
...
The **decision rules** tell coders what to do with the cases that do not sort themselves. This is where the edge cases from your observation log earn their keep. Every ambiguous case you wrote down becomes a rule.
...
Here is a draft codebook for the running example, the study the prospectus in Chapter 6 committed to and the observation supplement sharpened. It has three variables, which is the floor for a workable study, and it is deliberately a draft: the special-cases section is thin because piloting has not happened yet.
