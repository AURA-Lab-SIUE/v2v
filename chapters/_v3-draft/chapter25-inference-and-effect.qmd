...
![Mean message length for each group with a ninety-five percent confidence interval, on an axis starting at zero.](../../images/fig25-1-means-ci.png){fig-alt="A dot-and-interval chart with two points on a vertical axis of message length running from zero to about 35 characters. Gaming channels average 29.5 characters and non-gaming channels 32.0. Both intervals are so short they are barely visible at this scale and the two do not overlap. The visual point is that both means sit close together and far above zero, so the gap between them is small next to the quantity being measured."}
