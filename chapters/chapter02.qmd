...
![](../images/ch02-infrastructure_3.jpg){fig-alt="A stylized rendering of overlapping geometric forms suggesting structural scaffolding."}
...
When everything is installed, open VSCode, create a new file, and try the following. Save it as `setup-check.qmd`, then click Quarto's Preview button or run `quarto preview setup-check.qmd` in the terminal.
...
The R chunk below calls a function from the v2v package.
If everything is installed correctly, it prints a confirmation.
...
**The audit trail begins at setup.** The undergrad edition configures the workspace to make coding accessible: install R, create a GitHub repo, connect it. The graduate edition reframes each of those steps as a *methodological commitment* rather than a tool choice.

**Git as epistemological infrastructure.** Every `git commit` is a time-stamped record of your analysis at a specific moment. Your entire analytical history is reproducible: another researcher can check out any commit and see exactly what your code did at that point. This is not a convenience feature — it is a standard of evidence. Graduate work in V2V treats `git log` as a portion of the methods section. The first commit — before any analysis code is written — should include your data, your codebook, and a `README` that describes the study.

**The renv.lock as a methods citation.** The `renv.lock` file records the exact version of every R package your analysis depends on. When you publish a white paper, the `renv.lock` is the citation for your computational environment. Without it, a reader attempting replication may get different results because a package updated between your run and theirs. Run `renv::init()` before your first line of analysis code and commit the resulting lock file before you collect your first data point.

**Required reading.** Wilson, G., Bryan, J., Cranston, K., Kitzes, J., Nederbragt, L., & Teal, T. K. (2017). Good enough practices in scientific computing. *PLOS Computational Biology*, *13*(6), e1005510. https://doi.org/10.1371/journal.pcbi.1005510

**Reflect.** (1) Run `git log --oneline` in a project you completed before learning version control. What would a reviewer want to see that you cannot now reconstruct? (2) Wilson et al. distinguish "good enough" practices from "best" practices. What is the practical cost of skipping `renv::init()` in a project you intend to publish?
:::
:::
