...
![](../images/ch14-measuring_2.jpg){fig-alt="A stylized rendering of stacked pages resolving into a single bound document."}
...
**Two additional deliverables.** The undergrad edition produces a Quarto-rendered portfolio demonstrating the full V2V workflow in one reproducible document. The graduate edition produces two further deliverables: a *white paper* structured for academic publication and a *conference poster* formatted for a communication research conference.

**The white paper vs. the portfolio.** The undergraduate portfolio is structured as a feature-style explainer for an educated general audience. A white paper follows IMRaD format (Introduction, Methods, Results, Discussion), uses APA 7th edition throughout, reports effect sizes and power statistics, and includes a pre-registration disclosure statement in the methods section. The discussion situates findings within the theoretical literature from Chapter 5 rather than narrating them for general readers.

**Methods section requirements for the white paper.** Graduate V2V methods sections include three elements the portfolio does not: (1) a data provenance statement (Chapter 9); (2) an intercoder reliability report — the κ value, training protocol, and threshold applied (Chapter 8); and (3) the pre-registration disclosure with OSF URL (Chapter 5). Each element is a transparency marker that signals to reviewers that the study meets graduate-level evidentiary standards.

**The conference poster.** A standard academic communication conference poster is 36 × 24 inches, landscape orientation, structured as: title/authors/institution; abstract (~100 words); background and hypotheses; condensed methods; key results with one or two `theme_v2v()` figures; discussion and implications; QR code linking to the OSF pre-registration and/or Quarto portfolio. The poster is your 90-second pitch.
...
**Reflect.** (1) Convert the key results and discussion from your undergraduate portfolio into IMRaD structure. What is lost? What must be added to meet white paper standards? (2) Design the layout of your 36 × 24 conference poster. Which single figure from your analysis best communicates your main finding in 30 seconds?
:::
:::
## References
