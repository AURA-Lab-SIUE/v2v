...
Suppose you sat down to code the chat fixture by hand, one message at a time, the codebook open beside you. Read the message, decide who it is aimed at, record the code, move to the next. At a brisk and probably unsustainable pace of one message every ten seconds, the 157,080 messages in the fixture would take over four hundred hours. The full 2018 collection, all twenty-two million messages, would outlast your degree.
...
Both rules are written down, which is what makes this checkable: `data-raw/v3/build_reliability.py` applies them to the first hundred messages of the fixture and writes `pilot_coding.csv`. Coder A flags 5 messages, coder B flags 17, and they both flag the same message exactly once. Their codes go into `v2v::reliability()`:
