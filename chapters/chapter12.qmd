...
![](../images/ch12-cause-effect_1.jpg){fig-alt="A stylized rendering of a scatter of dots resolving into a clear rising line."}
...
msglen |>
  ...
  summarise(
    n      = n(),
    mean   = mean(message_length),
    median = median(message_length),
    sd     = sd(message_length),
    .groups = "drop"
  ) |>
  v2v::pretty_table()
```
...
**Effect size reporting with every comparison.** The undergrad edition focuses on visualization as communication: a good figure conveys the pattern in the data without distortion. That standard holds at the graduate level. But graduate-level descriptive reporting adds a further requirement: every figure that displays a comparison must include the effect size of that comparison.

A bar chart showing that one group scored higher than another is not a scientific claim — it is a visual impression. The effect size quantifies the magnitude of that difference in a scale-invariant unit. Cohen's *d* reports how many standard deviations apart two group means are; Cramer's *V* reports the strength of association between two categorical variables after accounting for marginal distributions. These values allow future researchers to use your descriptives as inputs for their power analyses — closing the cumulative science loop that Chapter 4 opened.

**Reporting conventions.** For every group comparison in a visualization, report the effect size in the figure caption or a companion table. For group means: Cohen's *d* with a 95% confidence interval. For cross-tabulations: Cramer's *V*. For correlations: *r* and its CI. The `v2v::run_t_test()` and `v2v::run_chi_square()` functions compute all of these automatically and return formatted output ready for the caption.

**Required reading.** Lakens, D. (2013). Calculating and reporting effect sizes to facilitate cumulative science: A practical primer for *t*-tests and ANOVAs. *Frontiers in Psychology*, *4*, 863. https://doi.org/10.3389/fpsyg.2013.00863

**Reflect.** (1) For two figures in your current analysis, report the effect size you would include in the caption. If one effect is small by Cohen's (1988) conventions, what does that imply for the interpretive claim you planned to make? (2) Lakens argues that routine effect size reporting would improve meta-analyses in communication research. What are the practical barriers to adopting this convention in a field where effect sizes are rarely reported in standard write-ups?
:::
:::
## References
