...
try {
    ...
    $d.Range("D2:D$last").Formula = "=IF(C2=""NA"","""",LEN(C2))"
    ...
    $spec = @(
        @("gaming mean all",     "=AVERAGEIFS($LEN,$LAB,""gaming"")",                      28.49, 2),
        @("gaming mean talk",    "=AVERAGEIFS($LEN,$LAB,""gaming"",$CMD,""talk"")",        29.88, 2),
        @("nongaming mean all",  "=AVERAGEIFS($LEN,$LAB,""nongaming"")",                   33.7,  2),
        @("nongaming mean talk", "=AVERAGEIFS($LEN,$LAB,""nongaming"",$CMD,""talk"")",     34.13, 2),
        @("gaming n talk",       "=COUNTIFS($LAB,""gaming"",$CMD,""talk"",$LEN,"">=0"")",  29389, 0),
        @("nongaming n talk",    "=COUNTIFS($LAB,""nongaming"",$CMD,""talk"")",            3396,  0),
        @("dev1 n",              "=COUNTIF($CH,""dev1"")",                                 1000,  0),
        @("dev1 commands",       "=COUNTIFS($CH,""dev1"",$CMD,""command"")",               480,   0),
        @("dev1 mean all",       "=AVERAGEIFS($LEN,$CH,""dev1"")",                         15.27, 2),
        @("dev1 mean talk",      "=AVERAGEIFS($LEN,$CH,""dev1"",$CMD,""talk"")",           24.53, 2)
    )
    ...
    foreach ($p in @(@("gap all", $g1, 5.21), @("gap talk only", $g2, 4.25))) {
        $ok = ($p[1] -eq $p[2])
        if (-not $ok) { $bad++ }
