...
try {
    ...
    foreach ($p in @(@("gap all", $g1, 5.22), @("gap talk only", $g2, 4.26))) {
        $ok = ($p[1] -eq $p[2])
        if (-not $ok) { $bad++ }
